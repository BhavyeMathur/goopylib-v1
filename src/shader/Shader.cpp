...
namespace gp {
    Shader::Shader(const char *vertexShaderSource, const char *fragmentShaderSource) : m_VertexShaderSource(
            vertexShaderSource),
        m_FragmentShaderSource(fragmentShaderSource) {
        GP_CORE_DEBUG("gp::Shader::Shader()");
    }

    shared_ptr<Shader> Shader::load(const char *vertShaderPath, const char *fragShaderPath) {
        return std::make_shared<Shader>(readFile(vertShaderPath).c_str(),
                                        readFile(fragShaderPath).c_str());
    }
