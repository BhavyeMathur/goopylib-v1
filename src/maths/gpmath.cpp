...
bool checkTriangleContains(Point point, Point a, Point b, Point c) {
    const float px_minus_ax = point.x - a.x;
    const float py_minus_ay = point.y - a.y;

    const bool s_ab = (b.x - a.x) * py_minus_ay - (b.y - a.y) * px_minus_ax > 0;
...
bool checkQuadContains(Point point, Point a, Point b, Point c, Point d) {
    const float px_minus_ax = point.x - a.x;
    const float py_minus_ay = point.y - a.y;
    const float cx_minus_ax = c.x - a.x;
    const float cy_minus_ay = c.y - a.y;

    const bool s_ab = (b.x - a.x) * py_minus_ay - (b.y - a.y) * px_minus_ax > 0;

    if ((cx_minus_ax * py_minus_ay - cy_minus_ay * px_minus_ax > 0 == s_ab) or ((c.x - b.x) * (point.y - b.y) - (c.y - b.y) * (point.x - b.x) > 0 != s_ab)) {

        const bool s_ac = cx_minus_ax * py_minus_ay - cy_minus_ay * px_minus_ax > 0;
