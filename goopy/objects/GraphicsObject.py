...
class GraphicsObject:
    ...
    def on_update(graphwin):
        ...
        for obj in GraphicsObject.resizing_objects:
            if obj.graphwin == graphwin and obj.drawn:
                if t - obj.resizing_start >= obj.resizing_time:
                    obj.is_resizing = False
                    GraphicsObject.resizing_objects.remove(obj)
                else:
                    args = ""
                    if obj.resizing_args is not None:
                        for arg in obj.resizing_args.split(","):
                            args += ", {}".format(arg)

                    obj.resize_factor(1 + obj.resizing_factor * ((t - obj.resizing_start) / obj.resizing_time),
                                     set_initial_size=False)
                    obj.resizing_update = t
                        ...
                        for bound in obj.resizing_bounds:
                            if obj.is_resizing[bound]:
                                if bound in ["left", "right"]:
                                    ...
                                    obj.set_outline_width(w, dir1)

                                    obj.resizing_bounds["top"].set_outline_width(w + obj.bounds_thickness, dir1)
                                    obj.resizing_bounds["bottom"].set_outline_width(w + obj.bounds_thickness, dir1)
