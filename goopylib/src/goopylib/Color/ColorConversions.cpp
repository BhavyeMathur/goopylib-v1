...
namespace {
    unsigned int digitToInt(char digit) {
...
namespace gp {
    ...
    namespace rgb {
        ...
        HSL toHSL(unsigned int red, unsigned int green, unsigned int blue) {
            ...
            if (cmax == redf) {
                return HSL{(unsigned int) round(60 * fmodf((greenf - bluef) / delta, 6)), s, L};
            }
            else if (cmax == greenf) {
                return HSL{(unsigned int) round(60 * (((bluef - redf) / delta) + 2)), s, L};
            }
            else {
                return HSL{(unsigned int) round(60 * (((redf - greenf) / delta) + 4)), s, L};
            }
        }
        ...
        HSV toHSV(unsigned int red, unsigned int green, unsigned int blue) {
            ...
            if (delta == 0) {
                ...
                h = (unsigned int) round(60 * fmodf((greenf - bluef) / delta, 6));
            }
            else if (cmax == greenf) {
                h = (unsigned int) round(60 * (((bluef - redf) / delta) + 2));
            }
            else {
                h = (unsigned int) round(60 * (((redf - greenf) / delta) + 4));
            }
    ...
    namespace cmyk {
        RGB toRGB(float cyan, float magenta, float yellow, float key) {
            key = 1 - key;
            return RGB{(unsigned int) round(255 * (1 - cyan) * key),
                       (unsigned int) round(255 * (1 - magenta) * key),
                       (unsigned int) round(255 * (1 - yellow) * key)};
        }
    ...
    namespace hsl {
        RGB toRGB(unsigned int hue, float saturation, float luminance) {
            ...
            if (hue < 60) {
                return RGB{(unsigned int) round(255 * (c + m)),
                           (unsigned int) round(255 * (x + m)),
                           (unsigned int) round(255 * m)};
            }
            else if (hue < 120) {
                return RGB{(unsigned int) round(255 * (x + m)),
                           (unsigned int) round(255 * (c + m)),
                           (unsigned int) round(255 * m)};
            }
            else if (hue < 180) {
                return RGB{(unsigned int) round(255 * m),
                           (unsigned int) round(255 * (c + m)),
                           (unsigned int) round(255 * (x + m))};
            }
            else if (hue < 240) {
