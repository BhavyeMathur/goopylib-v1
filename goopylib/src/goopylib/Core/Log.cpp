...
namespace gp {
    ...
    std::shared_ptr<spdlog::logger> Log::s_PythonLogger;
    ...
    void Log::init() {
        ...
        s_CoreLogger = std::make_shared<spdlog::logger>("GOOPYLIB", begin(logSinks), end(logSinks));
        spdlog::register_logger(s_CoreLogger);
        s_CoreLogger->set_level(spdlog::level::trace);
        s_CoreLogger->flush_on(spdlog::level::trace);

        s_PythonLogger = std::make_shared<spdlog::logger>("PYTHON", begin(logSinks), end(logSinks));
