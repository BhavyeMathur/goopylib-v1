...
class WindowMethods(unittest.TestCase):
    ...
    def test_x_position(self):
        self.window.x_position = 200
        self.assertEqual(self.window.x_position, 200)

    def test_x_position_errors(self):
        initial_value = self.window.x_position

        with self.assertRaises(TypeError):
            self.window.x_position = 102.2

        self.assertEqual(self.window.x_position, initial_value)

    # Height

    def test_y_position(self):
        self.window.y_position = 205
        self.assertEqual(self.window.y_position, 205)

    def test_y_position_errors(self):
        initial_value = self.window.y_position

        with self.assertRaises(TypeError):
            self.window.y_position = 102.2

        self.assertEqual(self.window.y_position, initial_value)
